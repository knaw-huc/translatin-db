...
WHERE
	...
	) <= ROUND(GREATEST(LENGTH(MT1.TITLE), LENGTH(MT2.TITLE)) * 0.3)
	
	-- No need to see each symmetrical pair (E_i, E_j) again as (E_j, E_i) (where i < j)
	AND (SUBSTRING(E1.LABEL, 2, LENGTH(E1.LABEL)-1))::int < (SUBSTRING(E2.LABEL, 2, LENGTH(E2.LABEL)-1))::int
...
select
	distinct on (
		(SUBSTRING(LABEL1, 2, LENGTH(LABEL1)-1))::int,
		(SUBSTRING(LABEL2, 2, LENGTH(LABEL2)-1))::int
	)
	label1, 'category_unused', label2
from main m1
where not exists (select from main where label2 = m1.label1)
order by
	(SUBSTRING(LABEL1, 2, LENGTH(LABEL1)-1))::int,
	(SUBSTRING(LABEL2, 2, LENGTH(LABEL2)-1))::int
	$$
) as ct(label text, peer1 text, peer2 text, peer3 text)
